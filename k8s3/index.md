...
答：K8s中对于`pod`资源对象的健康状态检测，提供了三类`probe（探针）`来执行对pod的健康监测：
...
1. 基于等值关系的，如：“=”、“”“==”、“！=”（注：“==”也是等于的意思，yaml文件中的matchLabels字段）；
2. 基于集合的，如：in、notin、exists（yaml文件中的matchExpressions字段）；
...
```
## 15、 DaemonSet资源对象的特性？
`DaemonSet`这种资源对象会在每个k8s集群中的节点上运行，并且每个节点只能运行一个pod，这是它和deployment资源对象的最大也是唯一的区别。所以，在其yaml文件中，不支持定义replicas，除此之外，与Deployment、RS等资源对象的写法相同。
```sh
它的一般使用场景如下：
